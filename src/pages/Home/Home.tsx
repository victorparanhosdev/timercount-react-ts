...
export function Home() {
  ...
  const { dataTime, isActiveId } = newCycleReducer;
  ...
  const ActiveNumber = dataTime.find(
    (dadostimer) => dadostimer.id === isActiveId
  );
  ...
  useEffect(() => {
    ...
    if (ActiveNumber) {
      interval = setInterval(() => {
        const CounterSeconds = differenceInSeconds(
          new Date(),
      ...
      <form onSubmit={handleSubmit(addDataForm)}>
        <div>
          ...
          <input
            ...
            type="number"
            id="box-number"
            {...register("numeroplay", { valueAsNumber: true })}
          />
        </div>
        {errors.numeroplay && <p>{errors.numeroplay.message}</p>}
        <button
          disabled={!isSubmitDisabled || isSubmitDisabled < 1}
          type="submit"
        >
